...
# import card2array
from .card2array import cards_one_hot, card_current_rank_one_hot, action_one_hot_code, card_red_rank_index, fix_length_action_sequence_one_hot
...
class WrapEnv:
    ...
    def _wrap_state(self):
        ...
        current_rank_one_hot = card_current_rank_one_hot(current_rank)
        current_rank_one_hot_batch = np.repeat(current_rank_one_hot[np.newaxis, :],
                                               num_legal_actions, axis=0)

        my_hand_cards = cards_one_hot(hand_card)
        ...
        last_action = action_one_hot_code(_last)
        last_action_batch = np.repeat(last_action[np.newaxis, :],
                                      num_legal_actions, axis=0)

        my_action_batch = np.zeros(last_action_batch.shape)
        for j, action in enumerate(legal_actions):
            my_action_batch[j, :] = action_one_hot_code(action)

        up_id = (current_player + 3) % 4
        down_id = (current_player + 1) % 4
        teammate_id = (current_player + 2) % 4

        up_played_cards = cards_one_hot(player_played_cards[up_id])
        down_played_cards = cards_one_hot(player_played_cards[down_id])
        teammate_played_cards = cards_one_hot(player_played_cards[teammate_id])
        ...
        red_rank_index = card_red_rank_index(current_rank)
        ...
        last_up_action = action_one_hot_code(last_up)
        ...
        last_teammate_action = action_one_hot_code(last_teammate)
        ...
        last_down_action = action_one_hot_code(last_down)
        ...
        z = fix_length_action_sequence_one_hot(sparse_action_sequence)
